...
class Admincp_account_groups extends MY_Controller {
	private $module = 'Admincp_account_groups';
	private $table = 'admin_groups';
				...
				if($this->input->post('hiddenIdAdmincp')!=0){
					...
					if($list_account){
						foreach($list_account as $val){
							...
							$this->db->update('admin_users',array('permission'=>substr($perm,1)));
						}
					}
				}
				print 'success';
		...
		if($perm=='permission-denied'){
			...
			if($this->db->update($this->table, $data)){
				...
				if($list_account){
					foreach($list_account as $val){
						$this->db->where('id',$val->id);
						$this->db->update('admin_users',array('status'=>$status));
					}
				}
			}
