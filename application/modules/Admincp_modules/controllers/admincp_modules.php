...
class Admincp_modules extends MX_Controller {
	...
	//			var_dump($this->admincp_account_groups_model);
				...
				foreach($list_group as $v){
					if($status==0){
						...
						if($pos !== false){
							...
							$this->db->where('id',$v->id);
							$this->db->update('admin_groups',array('permission'=>$new_perm));
						}
					}else{
						...
						if($pos !== false){
							...
							$this->db->where('id',$v->id);
							$this->db->update('admin_groups',array('permission'=>$v->permission.','.$this->input->post('id').'|'.$new_perm));
						}
					}
				}
				...
				foreach($list_account as $v){
					if($status==0){
						...
						if($pos !== false){
							...
							$new_perm = str_replace($sub_str,'',$v->permission);
							$this->db->where('id',$v->id);
							$this->db->update('admin_users',array('permission'=>$new_perm));
						}
					}else{
						...
						if($pos !== false){
							$new_perm = substr($v->permission,$pos+2,3);
							$this->db->where('id',$v->id);
							$this->db->update('admin_users',array('permission'=>$v->permission.','.$this->input->post('id').'|'.$new_perm));
						}
					}
				}
			}
