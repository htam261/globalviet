        ...
        <div class="row_text_field_first">
            <table cellspacing="0" cellpadding="0" border="0" width="100%">
                <tr>
                    ...
                    <td class="right_text_field"><input <?php if (isset($result->status)){
                                                        if ($result->status == 1){ ?>checked="checked" <?php }
                                                        }else{ ?>checked="checked"<?php } ?> type="checkbox"
                                                        class="custom_chk" name="statusAdmincp"/></td>
                </tr>
            </table>
        </div>
            ...
            <table cellspacing="0" cellpadding="0" border="0" width="100%">
                <tr>
                    ...
                    <td class="right_text_field">
                        <select onChange="getPerm(this.value)" class="custom_select" name="groupAdmincp"
                                id="groupAdmincp">
                            <?php
                            if ($list_group) {
                                foreach ($list_group as $value) {
                                    ?>
                                    <option <?php if (isset($result->group_id)){
                                            if ($result->group_id == $value->id){ ?>selected="selected"<?php }
        ...
        <div class="row_text_field">
            <table cellspacing="0" cellpadding="0" border="0" width="100%">
                <tr>
                    ...
                    <td class="right_text_field"><input value="<?php if (isset($result->username)) {
                            print $result->username;
                        } else {
                            print '';
                        } ?>" type="text" name="usernameAdmincp" id="usernameAdmincp"/></td>
                </tr>
            </table>
        </div>
                    ...
                    <td class="right_text_field" id="ajax_perm">
                        <?php if ($id == 0) { ?>
                            <script type="text/javascript">$(document).ready(function () {
                                    getPerm($('#groupAdmincp').val(), 0);
                                })</script>
                        <?php }else{ ?>
                            <script type="text/javascript">$(document).ready(function () {
                                    getPerm($('#groupAdmincp').val(), '<?=$result->permission?>');
                                })</script>
                        <?php } ?>
                    </td>
                </tr>
